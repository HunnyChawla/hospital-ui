...
export function ExaminationTabs({
  ...
  const { viewMode, setViewMode, useMergedVisionView, setUseMergedVisionView } = useExaminationViewPreference();
  const { hiddenTabs } = useExaminationViewContext();
  const containerRef = useRef<HTMLDivElement>(null);

  // If merged view is enabled, auto redirect from hidden tabs to vision
  useEffect(() => {
    if (useMergedVisionView && ["ar_data", "refraction", "current_specs"].includes(activeTab)) {
      onTabChange("vision");
    }
  }, [useMergedVisionView, activeTab, onTabChange]);

  const visibleTabs = useMemo(() => {
    if (useMergedVisionView) {
      return tabs
        .filter((t) => t.id !== "ar_data" && t.id !== "refraction" && t.id !== "current_specs")
        .map((t) => (t.id === "vision" ? { ...t, label: "Vision & Refraction" } : t));
    }
    return tabs;
  }, [useMergedVisionView]);
        ...
        <div className="flex items-center justify-between gap-2 px-2 py-1.5 sm:px-3">
          ...
          <div className="flex items-center gap-2 border-l border-slate-200 pl-2 sm:pl-3">
            <button
              ...
              className={clsx(
                "flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-semibold shadow-sm transition-all duration-200 border",
                useMergedVisionView
                  ? "bg-sky-50 text-sky-700 border-sky-200 hover:bg-sky-100"
                  : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50 hover:text-slate-900"
              )}
              title={useMergedVisionView ? "Switch to separate tabs view" : "Switch to merged vision & refraction view"}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="14"
              ...
              <span className="hidden lg:inline">{useMergedVisionView ? "Merged Layout" : "Tabbed Layout"}</span>
              <span className="lg:hidden">{useMergedVisionView ? "Merged" : "Tabbed"}</span>
            </button>
          </div>
