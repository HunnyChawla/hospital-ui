...
import { useExaminationViewContext, SECTION_CONFIG } from "@/context/ExaminationViewContext";
...
export function TabVisibilitySettings({ isOpen, onClose }: TabVisibilitySettingsProps) {
    const { hiddenTabs, toggleTabVisibility } = useExaminationViewContext();
                ...
                <div className="p-4 max-h-[60vh] overflow-y-auto">
                    ...
                    <div className="space-y-2">
                        {SECTION_CONFIG.map((section) => {
                            const isVisible = !hiddenTabs.includes(section.id);
                            const Icon = section.icon;

                            return (
                                <button
                                    key={section.id}
                                    onClick={() => toggleTabVisibility(section.id)}
                                    className={clsx(
