...
import { bulkCreateAdvices } from "@/redux/advicesSlice";
import { CreateAdviceRequest } from "@/services/advicesApi";
import { toast } from "sonner";
import { getErrorMessage } from "@/utils/errorHandler";
import { Modal } from "../common/Modal";
import { Upload, FileJson, AlertCircle } from "lucide-react";

type AdviceBulkImportModalProps = {
    isOpen: boolean;
    onClose: () => void;
    onSuccess: () => void;
    tenantId?: string;
...
export function AdviceBulkImportModal({ isOpen, onClose, onSuccess, tenantId }: AdviceBulkImportModalProps) {
    ...
    const handleSubmit = async () => {
        ...
        // Validate JSON
        let parsedData: CreateAdviceRequest[];
        try {
            ...
            for (let i = 0; i < parsedData.length; i++) {
                ...
                if (!item.advice_name || !item.category) {
                    setValidationError(
                        `Item at index ${i} is missing required fields (advice_name, category)`
                    );
                    return;
                }
            }
        ...
        try {
            ...
            await dispatch(bulkCreateAdvices({ advices: parsedData, tenantId })).unwrap();
            toast.success(`Successfully imported ${parsedData.length} advices`);
            setJsonInput("");
            onSuccess();
        } catch (error) {
    ...
    const exampleJson = `[
  {
    "advice_name": "Avoid rubbing eyes",
    "category": "General",
    "is_active": true,
    "applicable_eye": "BOTH"
  },
  {
    "advice_name": "Use protective glasses",
    "category": "Post-Op",
    "is_post_op": true,
    "is_active": true
  }
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Bulk Import Advices" size="lg">
            <div className="space-y-4 p-1">
                <div className="rounded-xl bg-blue-50 border border-blue-200 p-3">
                    <div className="flex gap-2">
                        ...
                        <div className="text-sm text-blue-800">
                            ...
                            <p className="text-xs">
                                Paste a JSON array of advice objects. Each object must have{" "}
                                <code className="bg-blue-100 px-1 rounded">advice_name</code> and{" "}
                                <code className="bg-blue-100 px-1 rounded">category</code>.
                                Optional fields: description, applicable_eye, is_post_op, is_active.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    >
                        ...
                        {isSubmitting ? "Importing..." : "Import Advices"}
                    </button>
                </div>
            </div>
        </Modal>
