...
export function AdvancePaymentModal({
  ...
  // Snapshot the surgery so the close transition has data to render with -
  // the parent nulls `surgery` in the same tick it flips `isOpen` off.
  const [displaySurgery, setDisplaySurgery] = useState<PlannedSurgery | null>(surgery);
  useEffect(() => {
    if (surgery) setDisplaySurgery(surgery);
  }, [surgery]);
  ...
  const surgeryId = displaySurgery?.id || null;
  ...
  const packagePrice = summary?.package_price ?? (displaySurgery?.package_price ? Number(displaySurgery.package_price) : null);
  ...
  if (!displaySurgery) return null;

  const isPackageFinalized = !!(displaySurgery.package_id || displaySurgery.package_price || summary?.package_price);
                ...
                <Dialog.Panel className="w-full max-w-xl transform overflow-hidden rounded-2xl bg-white shadow-2xl transition-all">
                  ...
                  <div className="flex items-center justify-between border-b border-slate-200 bg-gradient-to-r from-emerald-50 to-teal-50 px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-emerald-200 bg-white shadow-2xs text-emerald-700">
                        <CreditCard className="h-5 w-5" />
                      </div>
                      <div>
                        <Dialog.Title className="text-lg font-bold text-slate-900">Collect Surgery Advance</Dialog.Title>
                        <p className="text-xs text-slate-500">
                          {displaySurgery.surgery_name} • {displaySurgery.patient_name || "Patient"} ({displaySurgery.patient_uhid || "No UHID"})
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="max-h-[82vh] overflow-y-auto p-6">
                    ...
                    {lastPayment ? (
                      <div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-6 text-center">
                        ...
                        <div className="mt-6 flex items-center justify-center gap-3">
                          <button
                            ...
                            className="flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-emerald-700 transition-colors"
                          >
                      ...
                      /* Package Not Finalized — persistent blocking banner, nothing else to show yet */
                      <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 text-center">
                        ...
                        <p className="mt-1 text-xs text-amber-700 max-w-md mx-auto">
                          A surgery package has not been selected for this surgery yet. Use the &quot;Plan&quot; action to select and
                          finalize a package before collecting advance payments.
                        </p>
                      </div>
                    ) : (
                            ...
                            <div className="max-h-28 overflow-y-auto space-y-1.5 pr-1">
                              {advancePayments.map((pay) => (
                                <div
                                  ...
                                  className="flex items-center justify-between rounded-lg border bg-white p-2 text-xs shadow-sm"
                                >
                                  <div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  placeholder="0.00"
                                  className={`w-full rounded-xl border pl-9 pr-3 py-2.5 text-sm outline-none transition-colors focus:ring-2 ${
                                    isExceedingBalance
                                      ? "border-rose-500 focus:border-rose-500 focus:ring-rose-100 bg-rose-50/30 text-rose-900"
                            ...
                            <div>
                              ...
                              <select
                                ...
                                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none transition-colors focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
                              >
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none transition-colors focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
                            />
                          </div>
                          ...
                          <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
                            ...
                            <button
                              ...
                              className="flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow hover:bg-emerald-700 disabled:opacity-50 transition-colors"
                            >
                              {collectAdvanceMutation.isPending ? (
                                <>
