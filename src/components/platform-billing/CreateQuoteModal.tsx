...
import { CreateQuoteRequest } from "@/types/platformBilling";
...
export function CreateQuoteModal({ isOpen, onClose, onSuccess }: CreateQuoteModalProps) {
  ...
  const onSubmit = async (values: CreateQuoteRequest) => {
    setSubmitting(true);
    try {
      // Set tax_rate to 0 if GST is not enabled
      const payload = {
        ...values,
        tax_rate: gstEnabled ? Number(values.tax_rate) : 0,
      };
      await platformBillingApi.quotes.create(payload);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Create Platform Quote" size="xl">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <label className="space-y-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 outline-none focus:border-sky-400 text-sm"
            />
            {errors.valid_until && <p className="text-xs text-rose-500">{errors.valid_until.message}</p>}
          </label>
        </div>
                ...
                <div className="flex-grow grid grid-cols-1 gap-3 sm:grid-cols-4">
                  <label className="sm:col-span-2 space-y-1">
                    ...
                    <input
                      ...
                      placeholder="e.g. Annual HMS Core License Fee"
                    />
                  </label>
                  <label className="space-y-1">
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-slate-700">Charge GST (Tax) on Quote</span>
            <button
              type="button"
              onClick={() => {
        ...
        <div className="flex justify-end gap-3 border-t border-slate-200 pt-4">
          ...
          >
            {submitting ? "Creating..." : "Save Quote"}
          </button>
        </div>
      </form>
    </Modal>
